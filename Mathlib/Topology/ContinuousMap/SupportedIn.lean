...
open TopologicalSpace Set Function UniformSpace
open scoped BoundedContinuousFunction Topology NNReal

variable {X V : Type*} [TopologicalSpace X] [TopologicalSpace V]

variable (X V) in
/-- The type of bundled continuous maps which vanish outside of a fixed compact set `K`. -/
structure ContinuousMapSupportedIn [Zero V] (K : Compacts X) extends C(X, V) where
...
scoped[CompactlySupported] notation (priority := 2000)
  "C_{" K "}(" X ", " V ")" => ContinuousMapSupportedIn X V K

open CompactlySupported

/-- `ContinuousMapSupportedInClass B X V K` states that `B` is a type of bundled continuous
functions with support in the compact set `K`. -/
class ContinuousMapSupportedInClass (B : Type*) (X V : outParam <| Type*)
    [TopologicalSpace X] [TopologicalSpace V] [Zero V]
    [FunLike B X V] (K : outParam <| Compacts X)
    extends ContinuousMapClass B X V where
  map_zero_on_compl (f : B) : EqOn f 0 Kᶜ

open ContinuousMapSupportedInClass

namespace ContinuousMapSupportedInClass

instance (B : Type*) (X V : outParam <| Type*)
    [TopologicalSpace X] [TopologicalSpace V] [Zero V]
    [FunLike B X V] (K : outParam <| Compacts X)
    [ContinuousMapSupportedInClass B X V K] :
