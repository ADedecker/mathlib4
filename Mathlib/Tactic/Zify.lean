...
import Mathlib.Tactic.ZifySimpsAttr
...
-/
def liftToZ (e : Expr) : Tactic.TacticM (Expr × Expr) := do
  let sl ← zifyExt.getTheorems
  --sl ← sl.add_simp `ge_iff_le, sl ← sl.add_simp `gt_iff_lt
  let ctx : Simp.Context := { simpTheorems := #[sl] }
  --Simp.withSimpTheorems #[sl] do
  let (e', prf, _) ← Tactic.simpLocation sl [] e
  return (e, e)
    --return (e', prf)
